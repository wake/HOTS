...
namespace Protype\Library\Metabox;
...
class Metabox {
  ...
  public function scripts() {
    ...
    if (
      ...
      wp_enqueue_style( 'wp-metabox-style', plugin_dir_url( __FILE__ ) . 'assets/style.css', array (), '1.0.7', null );
      wp_enqueue_script( 'wp-metabox-script', plugin_dir_url( __FILE__ ) . 'assets/script.js', array( 'jquery' ), '1.0.7', true );
    }
  }
   ...
   * @param string $meta
   */
  public function get_pdf_info( $field, $meta ) {
    global $post;

    $basename = basename($meta);

    echo sprintf(
      '<div id="%s" class="%s"><a download style="
        ...
        color: #fff;" target="_blank" href="%s">
          <span class="dashicons dashicons-pdf" style="width: 16px; font-size: 18px;"></span>
          <span class="pdf-name">%s</span>
        </a></div>',
      esc_attr( sprintf( 'js-%s-pdf-info', $field['id'] ) ),
      esc_attr( sprintf( '%s %s', $this->get_block_element_class_with_namespace( 'pdf-info', false ), empty( $meta ) ? 'is-hidden' : '' ) ),
      esc_attr( $meta ),
      esc_attr( strlen ($basename) > 20 ? substr ($basename, 0, 20) . '...' : $basename ),
    );
  }
  ...
  public function addPDF( $args, $repeater = false ) {
    $field = array_merge( array( 'type' => 'pdf' ), $args );
    if ( ! $repeater ) {
      $this->_fields[] = $field;
    } else {
      return $field;
  ...
  public function show_field_pdf( $field, $meta ) {
    $this->before_field( $field, $meta ); // pass in $meta for preview image
    echo sprintf(
      '<input type="hidden" id="%s" name="%s" value="%s">',
      esc_attr( 'pdf-' . $field['id'] ),
      esc_attr( $field['id'] ),
      ( isset( $meta ) ? $meta : '' )
    );
    echo sprintf(
      '<a class="%s button" data-hidden-input="%s" style="margin-top: 10px;">%s</a>',
      esc_attr( sprintf( 'js-%s-pdf-upload-button', self::BLOCK_NAMESPACE ) ),
      esc_attr( $field['id'] ),
      esc_html( sprintf( '%s', empty( $meta ) ? '上傳檔案' : '重新上傳' ) )
    );

    echo sprintf(
      '<a class="button %s" style="margin-left: 10px; margin-top: 10px;" onclick="
        jQuery(\'#%s\').val (\'\');
        jQuery(\'#%s\').css (\'opacity\', 0.3);
      " style="margin-top: 10px;">%s</a>',
      empty( $meta ) ? 'is-hidden' : '',
      esc_attr( 'pdf-' . $field['id'] ),
      esc_attr( sprintf( 'js-%s-pdf-info', $field['id'] ) ),
      esc_html( '取消檔案' )
    );
  ...
  public function show_field_Editor( $field, $meta ) {
    $this->before_field( $field );
    wp_editor( $meta, $field['id'], ['textarea_rows' => 10] );
    $this->after_field();
  }
